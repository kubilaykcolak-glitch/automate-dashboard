...
| `usage.ts` | `currentMonthKey()`, `getUserPlan()`, `recordTokenUsage()`, `getMonthlyTokenSummary()`, `recordWebSearches()`, `getMonthlyWebSearchUsage()`, `checkAndRecordRateLimit()`, `FREE_PLAN_MONTHLY_TOKEN_BUDGET=500_000`, `PAID_PLAN_MONTHLY_TOKEN_BUDGET=5_000_000`, `RATE_LIMIT_MAX_REQUESTS=10`/min | Server-only. **Single-axis billing**: tokens. `getMonthlyTokenSummary()` returns the per-user monthly snapshot with `remaining` and `overageTokens`; chat routes return 429 + `code: "token_budget_exceeded"` once `totalTokens >= budget`. Per-minute rate limit is separate (abuse guard, not billing). See `docs/TOKEN_BILLING.md`. |
